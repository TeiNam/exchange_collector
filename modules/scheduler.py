...
class SchedulerThread(threading.Thread):
    def __init__(self, run_immediately=False):
        ...
        # 정기 수집·알림 시간 (서울 외환시장 마감 15:30 직후 종가성 대표값 확보)
        self.schedule_time = "15:40"
        self.run_immediately = run_immediately

    def run(self):
        self.is_running = True

        # 스케줄 등록 (수집 + 매수 신호 분석/알림을 노티파이어 한 잡에서 처리)
        ...
        logger.info(f"실행 시간: 매일 {self.schedule_time} KST (환율 수집 + 매수 신호 알림)")
